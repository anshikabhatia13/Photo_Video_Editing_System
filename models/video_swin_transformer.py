...
def window_partition(x, window_size):
    """
    Args:
        x: (B, D, H, W, C) => (Batch,Depth,Height,Width,Channels)
        window_size (tuple[int]): window size

    Returns: The partitioned windows
        windows: (B*num_windows, window_size*window_size, C)
        
        x.view(...): Reshapes the tensor to prepare for partitioning.`
        permute: Rearranges the dimensions of the tensor.
        contiguous: Ensures the tensor is stored in contiguous memory.
        reduce(mul, window_size): Computes the product of the window dimensions.
    """
    B, D, H, W, C = x.shape
...
def window_reverse(windows, window_size, B, D, H, W):
    ...
    Args:
        ...
        B, D, H, W: Dimensions of the original input tensor.
